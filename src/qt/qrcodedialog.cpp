...
{
    QString ret = QString("mintcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MTC unit input in lnReqAmount, we generate the URI with MTC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(MintcoinUnits::format(MintcoinUnits::MTC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
