...
// Copyright (c) 2009-2012 The Mintcoin developers
// Copyright (c) 2011-2012 Litecoin Developers
// Copyright (c) 2013 MintCoin Developers
...
uint256 hashGenesisBlock("0x43cac31596223882a9123adcb93639e5853ae9cccbbe54599bfd36b503a67fe4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MintCoin: starting difficulty is 1 / 2^12
...
static const int64 nTargetTimespan = 4 * 60 * 60; // MintCoin: every 4 hours
static const int64 nTargetSpacing = 180; // MintCoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // MintCoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal mintcoin miner
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4540f8174f349ef554b42574beda7c923726233eca926c0af73e1a67729e2a38");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "21/Dec/2013 Mintcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xaab28ad3188ba16834aa75608c43d718c717ec5b9b44b00924f0380acff691a9"));
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // MintCoind: Reduce the exempted free transactions to 500 bytes (from Mintcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MintcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("MintcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadMintcoinMiner(void* parg);

static bool fGenerateMintcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static MintcoinMiner(CWallet *pwallet)
{
    printf("MintcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("mintcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateMintcoins)
    {
        if (fShutdown)
            return;
        ...
        printf("Running MintcoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadMintcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        MintcoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadMintcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadMintcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadMintcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateMintcoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateMintcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateMintcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d MintcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadMintcoinMiner, pwallet))
                printf("Error: CreateThread(ThreadMintcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
