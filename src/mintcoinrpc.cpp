...
// Copyright (c) 2013 The MintCoin Developers
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "MintCoin server has now stopped running!";
}
...
// MintCoin: Return average network hashes per second based on last number of blocks.
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateMintcoins(fGenerate, pwalletMain);
    return Value::null;
}
...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    return CMintcoinAddress(keyID).ToString();
}


CMintcoinAddress GetAccountAddress(string strAccount, bool bForceNew=false)
{
    ...
    return CMintcoinAddress(account.vchPubKey.GetID());
}
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    CMintcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid MintCoin address");
...
Value getaddressesbyaccount(const Array& params, bool fHelp)
{
    ...
    Array ret;
    BOOST_FOREACH(const PAIRTYPE(CMintcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        const CMintcoinAddress& address = item.first;
...
Value signmessage(const Array& params, bool fHelp)
{
    ...
    CMintcoinAddress addr(strAddress);
    if (!addr.IsValid())
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    // MintCoin address
    CMintcoinAddress address = CMintcoinAddress(params[0].get_str());
    CScript scriptPubKey;
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid MintCoin address");
...
Value sendfrom(const Array& params, bool fHelp)
{
    ...
    string strAccount = AccountFromValue(params[0]);
    CMintcoinAddress address(params[1].get_str());
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid MintCoin address");
    int64 nAmount = AmountFromValue(params[2]);
    int nMinDepth = 1;
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    set<CMintcoinAddress> setAddress;
    vector<pair<CScript, int64> > vecSend;

    int64 totalAmount = 0;
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        CMintcoinAddress address(s.name_);
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid MintCoin address:")+s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    ...
    return CMintcoinAddress(innerID).ToString();
}
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    // Tally
    map<CMintcoinAddress, tallyitem> mapTally;
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        const CWalletTx& wtx = (*it).second;
    ...
    BOOST_FOREACH(const PAIRTYPE(CMintcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        map<CMintcoinAddress, tallyitem>::iterator it = mapTally.find(address);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret)
{
    ...
    if ((!listSent.empty() || nFee != 0) && (fAllAccounts || strAccount == strSentAccount))
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& s, listSent)
        {
            ...
            entry.push_back(Pair("address", CMintcoinAddress(s.first).ToString()));
    ...
    if (listReceived.size() > 0 && wtx.GetDepthInMainChain() >= nMinDepth)
    {
        BOOST_FOREACH(const PAIRTYPE(CTxDestination, int64)& r, listReceived)
        {
            ...
            if (fAllAccounts || (account == strAccount))
            {
                ...
                entry.push_back(Pair("address", CMintcoinAddress(r.first).ToString()));
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; MintCoin server stopping, restart to run with encrypted wallet";
}
...
public:
    ...
    Object operator()(const CScriptID &scriptID) const {
        ...
        BOOST_FOREACH(const CTxDestination& addr, addresses)
            a.push_back(CMintcoinAddress(addr).ToString());
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // MintCoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 12635));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "12635")))
